...
- [x] 9.1 RED/GREEN: extend `tests/tui-model-profiles.test.mjs` for inline `models list/show`, picker-driven `profile show` / `switch` / `delete`, and field-form `set` / `profile create` flows with no fabricated commands.
- [x] 9.2 Update `scripts/lib/tui/model-profiles-adapter.mjs`, `scripts/lib/tui/screens/model-profiles.mjs`, `scripts/lib/tui/actions/forms.mjs`, `scripts/lib/tui/actions/definitions.mjs`, and `scripts/tui.mjs` for read-only action lists, picker/text forms, confirmations, output panels, and refresh-after-mutation.
- [x] 9.3 Update `tests/tui-docs.test.mjs`, `README.md`, `apply-progress.md`, and `verify-report.md`; rerun focused tests, full `pnpm test`, and bounded forced-TTY smoke for inline output, cancel, confirm, and post-mutation refresh.
...
| `README.md` | Modified | Documented Configuration/Status action-list keyboard flow, inline doctor output, init checkbox choices, and confirm-before-update behavior. |
...
| `tests/tui-model-profiles.test.mjs` | Modified | Added PR13 strict-TDD coverage for inline `models list/show`, picker-driven `profile show` / `switch` / `delete`, field-form `set` / `profile create`, confirmation flow, cancel recovery, and refreshed screen state. |
| `scripts/lib/tui/model-profiles-adapter.mjs` | Modified | Added manifest-backed PR13 Model Profiles interactive action definitions for list/show/switch/set/create/delete, including picker and mixed-field form metadata. |
| `scripts/lib/tui/actions/definitions.mjs` | Modified | Extended shared form validation to accept picker and mixed-field model-profile forms while keeping manifest allowlist enforcement. |
| `scripts/lib/tui/actions/forms.mjs` | Modified | Added generic picker and mixed-field form state helpers, text-entry/backspace support, and reusable submit/input extraction for PR13. |
| `scripts/tui.mjs` | Modified | Added generic picker/field-form rendering and input handling so Model Profiles actions support inline reads, confirmations, Esc cancel, and bounded output recovery. |
| `scripts/lib/tui/screens/model-profiles.mjs` | Modified | Added explicit interactive notes about inline reads, picker/text forms, and post-mutation refresh behavior. |
| `tests/tui-docs.test.mjs` | Modified | Locked README guidance for Model Profiles inline actions, picker/text forms, and refresh-after-mutation behavior. |
| `README.md` | Modified | Documented Model Profiles inline actions, picker/text forms, and immediate refresh behavior after successful mutations. |
...
| `tests/tui-model-profiles.test.mjs` | Modified | Added PR13 review-fix regressions for empty set/create validation, explicit `--allow-unknown` preservation, typed delete confirmation mismatch/cancel/success, and sanitized model-profile rendering surfaces. |
| `scripts/lib/tui/model-profiles-adapter.mjs` | Modified | Marked mutable model/profile text fields required and added typed-match delete confirmation metadata while keeping manifest-backed argv builders. |
| `scripts/lib/tui/actions/forms.mjs` | Modified | Added shared field-validation and typed-confirmation helpers, cleared stale validation messages on edits, and sanitized action metadata shown in bounded output panels. |
...
| `scripts/tui.mjs` | Modified | Blocked empty form submissions before argv resolution, added typed delete-confirm input handling, and sanitized confirmation/form rendering for user-controlled strings. |
...
| 8.3 | `tests/tui-shell.test.mjs` | Unit | ✅ `pnpm test -- tests/tui-shell.test.mjs` → 191/191 baseline before PR12 keyboard-flow updates | ✅ Added failing shell-level guidance tests first for section action help, form cancel copy, and off-Home no-action responsiveness | ✅ `pnpm test -- tests/tui-configuration.test.mjs tests/tui-status.test.mjs tests/tui-shell.test.mjs tests/tui-actions.test.mjs` → 196/196 after README/TUI guidance updates | ✅ Covered action-list navigation help, form focus bounds, Esc cancel copy, and a route with zero interactive actions | ✅ Kept the user-facing guidance terse in README while reusing the shell renderers for the visible text cues |
...
| 9.1 | `tests/tui-model-profiles.test.mjs` | Unit | ✅ `pnpm vitest run tests/tui-model-profiles.test.mjs` → 7/7 baseline before PR13 edits | ✅ Added failing assertions first for missing Model Profiles interactive actions, inline output, picker forms, and mutating flows before implementation | ✅ `pnpm vitest run tests/tui-model-profiles.test.mjs` → 9/9 after PR13 implementation | ✅ Covered inline list/show, picker-based `profile show` / `switch` / `delete`, field-form `set` / `profile create`, cancel recovery, and refresh-after-mutation | ✅ Reused isolated config fixtures plus a focused action executor so the tests stay behavior-first without mutating user config |
| 9.2 | `tests/tui-model-profiles.test.mjs` + `tests/tui-shell.test.mjs` | Unit | ✅ `pnpm vitest run tests/tui-actions.test.mjs tests/tui-shell.test.mjs` → 24/24 baseline before shared-form edits | ✅ Added failing expectations first for picker/field form metadata, generic modal rendering, and Model Profiles action execution before extending the shared framework | ✅ `pnpm vitest run tests/tui-docs.test.mjs tests/tui-shell.test.mjs tests/tui-actions.test.mjs tests/tui-model-profiles.test.mjs` → 35/35 after wiring shared picker/field forms and Model Profiles actions | ✅ Covered form rendering, Esc cancel, output close, shared framework regressions, and refreshed route state after successful mutations | ✅ Kept the implementation in shared form helpers plus the Model Profiles adapter so Configuration/Status behavior stayed green |
| 9.3 | `tests/tui-docs.test.mjs` + bounded PTY smoke | Mixed | ✅ `pnpm vitest run tests/tui-docs.test.mjs tests/tui-shell.test.mjs tests/tui-actions.test.mjs tests/tui-model-profiles.test.mjs` → 35/35 before final evidence sync | ✅ Added README contract assertions first for Model Profiles inline actions, picker/text forms, and refresh behavior before updating docs/evidence | ✅ `pnpm test` → 201/201 plus bounded forced-TTY smoke for inline output, confirm, cancel, and refreshed active profile | ✅ Covered inline `models list`, switched active profile to `fallback`, canceled a delete picker, and preserved keyboard-only exit flow in a temp config fixture | ➖ Verification/docs slice only |

### PR13 Review Fix Evidence
| Finding | Test File | Layer | Safety Net | RED | GREEN | TRIANGULATE | REFACTOR |
|---------|-----------|-------|------------|-----|-------|-------------|----------|
| Block empty `models set` / `models profile create` text submissions and keep focus recoverable | `tests/tui-model-profiles.test.mjs` | Unit | ✅ `pnpm vitest run tests/tui-model-profiles.test.mjs tests/tui-actions.test.mjs tests/tui-shell.test.mjs` → 33/33 baseline before the PR13 review-fix edits | ✅ Added failing form-submission regressions first for empty model/profile text plus explicit `--allow-unknown` preservation | ✅ `pnpm vitest run tests/tui-model-profiles.test.mjs tests/tui-actions.test.mjs tests/tui-shell.test.mjs` → 37/37 after required-field validation landed | ✅ Covered empty `set`, empty `profile create`, recoverable focus on the invalid field, and model text that stays positional unless the explicit toggle is enabled | ✅ Kept the fix in shared field-validation helpers plus adapter metadata so existing picker flows stayed unchanged |
| Require typed profile-name confirmation before `models profile delete` executes | `tests/tui-model-profiles.test.mjs` | Unit | ✅ `pnpm vitest run tests/tui-model-profiles.test.mjs tests/tui-actions.test.mjs tests/tui-shell.test.mjs` → 33/33 baseline before the PR13 review-fix edits | ✅ Added failing mismatch/cancel/success regressions first for delete confirmation text before execution | ✅ `pnpm vitest run tests/tui-model-profiles.test.mjs tests/tui-actions.test.mjs tests/tui-shell.test.mjs` → 37/37 after typed confirmation handling landed | ✅ Covered mismatch keeps execution blocked, Esc cancels safely, and sanitized visible text still matches the selected profile on success | ✅ Reused the existing picker-to-confirm pipeline by adding confirmation metadata instead of inventing a one-off delete form |
...
- `AFERGON_AI_FORCE_TTY=1 ./bin/afergon-ai tui` via bounded PTY smoke ✅ temp-fixture run produced inline `models list` output, confirmed `switch` to `fallback`, canceled a later picker safely with `Esc`, refreshed the Model Profiles section state, and exited with `q`
- `pnpm vitest run tests/tui-model-profiles.test.mjs` ❌ 9 passed / 4 failed after adding PR13 review-fix regressions first for empty field validation, typed delete confirmation, and sanitized rendering (RED confirmed)
...
None — implementation matches the final PR13 risk-warning scope and reused the existing shared sanitizer.

### Issues Found
`./bin/afergon-ai doctor --opencode` still reports pre-existing local OpenCode registration gaps outside this slice. PR13 does not change that environment issue.
...
- Current work unit: PR13 final risk warning — Configuration/Status render sanitization
- Boundary: starts after the verified PR13 Model Profiles review-fix slice and ends with Configuration/Status screen render sanitization plus regression coverage only; no new actions, routes, or features
- Estimated review budget impact: minimal follow-up diff limited to two renderers, two test files, and apply-progress evidence
