...
- [x] Both named frontmatter declarations, MANIFEST entries, and persisted complete permission objects remain exactly equal to the approved policies.
- [x] Direct evaluation of the persisted `afg-debate` write map allows `openspec/debate/debate-summary-agent-permissions.md` and denies `openspec/debate/notes.md`.
- [x] Separate missing-`afg-debate.md` and missing-`afergon-ai.md` test cases name the exact file and preserve `opencode.json` bytes.
- [x] Complete-object parity tests still detect added, removed, or changed permissions with focused diagnostics.
- [x] Unrelated agent permissions/metadata and named-agent non-permission metadata remain unchanged.
- [x] The PR delivers one resolvable canonical task/spec/plan/result chain and no superseded task or timestamped backup as an active input.
- [x] RESULT accurately records remediation evidence and all delivered implementation/correction commits without a stale or impossible SHA claim.
- [x] Focused tests, full tests, typecheck, build, shell syntax, and diff checks pass.
- [x] The existing branch is pushed without force and PR #70 remains open on that head.
- [x] PR #70 has the exact required title, `Closes #71`, nonclosing `Refs #67`, no closing keyword for #67, preserved non-type labels, and exactly one type label: `type:bug`.

## Verification

- [x] Tests: `pnpm build && pnpm exec vitest run tests/model-profiles.test.ts --no-file-parallelism -t "OpenCode registrar behavior"`
- [x] Tests: `pnpm exec vitest run tests/model-profiles.test.ts --no-file-parallelism`
- [x] Tests: `pnpm test`
- [x] Build: `pnpm typecheck && pnpm build`
- [x] Additional Evidence: `bash -n scripts/register-opencode-agents.sh && git diff --check`
- [x] Additional Evidence: `git status --short`, `git diff --stat`, `git diff --name-status`, `git diff <base>...HEAD --stat`, and focused diffs prove bounded scope and no generated/unrelated changes.
- [x] Additional Evidence: canonical-path inspection proves every source reference resolves and superseded split tasks/timestamped backups are absent from the delivered PR file set.
- [x] Additional Evidence: `git log --oneline <base>..HEAD` and `gh pr view 70 --json commits,files` agree with RESULT's delivered-commit/file inventory.
- [x] Additional Evidence: final `gh pr view 70 --json title,body,labels,headRefName,state,url` proves title, linkage, branch reuse, open state, non-type-label preservation, and sole `type:bug`.
- [x] Rule Compliance: RESULT records baseline, focused per-case runs, full verification, characterization-vs-RED status, lowest-sufficient changes, commit boundaries, and PR evidence.
