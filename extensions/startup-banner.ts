				...
				render(width: number): string[] {
					...
					if (width >= 90) {
						...
						const gap = Math.max(
							...
							width -
								4 -
								visibleLength("GIT " + branch) -
								visibleLength("SKILLS " + `${skills.length} loaded`),
						);
						out.push(
							truncateToWidth(
					...
					out.push(
						truncateToWidth("  " + statLine("PATH", path, opacity), width, ""),
					);
					out.push("");
